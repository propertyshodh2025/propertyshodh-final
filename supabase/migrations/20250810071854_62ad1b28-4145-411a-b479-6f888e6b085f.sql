-- Leads backfill and triggers for inquiries and saved activities
...
BEGIN
  IF NEW.activity_type = 'property_saved' AND NEW.property_id IS NOT NULL THEN
    ...
    INSERT INTO public.leads (
      ...
      'manual', NEW.id, COALESCE(uname, 'User'), uphone, uemail, NEW.property_id, prop_title, 'new', 'medium', ARRAY['property_saved']
    )
    ON CONFLICT DO NOTHING;
  END IF;
  RETURN NEW;
